#!/usr/bin/env python3
"""
Generate comparison data for Exact vs Greedy solutions using Part D additional test cases.
Creates both a CSV file and a line graph.
"""
...
GREEDY_SCRIPT = os.path.join(PROJECT_ROOT, "approx_solution", "cs412_longestpath_approx.py")
...
PNG_FILE = os.path.join(PROJECT_ROOT, "part_D", "exact_vs_greedy_comparison.png")
PDF_FILE = os.path.join(PROJECT_ROOT, "part_D", "exact_vs_greedy_comparison.pdf")
...
def generate_data():
    """Generate comparison data and save to CSV."""
    ...
    print(f"Generating comparison data...")
    print()
    ...
    for test_file in test_files:
        ...
        # Run Greedy
        greedy_ok, greedy_time, greedy_len = run_solver(GREEDY_SCRIPT, test_path, GREEDY_TIMEOUT)
        if not greedy_ok:
            print("GREEDY ERROR")
            continue

        # Calculate quality percentage
        quality_pct = (greedy_len / exact_len * 100.0) if exact_len > 0 else 0.0
        ...
        data.append({
            ...
            "greedy_value": greedy_len,
            "quality_percent": quality_pct,
        })

        print(f"✓ (E:{exact_time:.3f}s, G:{greedy_time:.3f}s, E:{exact_len}, G:{greedy_len})")

    # Save to CSV
    with open(CSV_FILE, "w", newline="") as csvfile:
        fieldnames = ["test_name", "vertices", "edges", "exact_time", "greedy_time", 
                     "exact_value", "greedy_value", "quality_percent"]
...
def plot_comparison(data):
    """Create line graph comparing Exact vs Greedy runtime."""
    if not data:
    ...
    greedy_times = [d["greedy_time"] for d in data]
...
def main():
    print("=" * 80)
    print("Exact vs Greedy Comparison - Part D Additional Test Cases")
    print("=" * 80)
    print()
